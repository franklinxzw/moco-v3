...
parser = argparse.ArgumentParser(description='MoCo Kinetics Linear Classification')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        ...
        linear_keyword = 'fc'
    model = torchvision_models.video.r3d_18()
    ...
    train_dataset = VideoDataset(video_folder= args.data, contrastive=False, train=True, transform=video_augmentation_train, stride=args.stride, clip_len=args.clip_len)
    ...
    val_dataset = VideoDataset(video_folder= args.data, contrastive=False, train=False, transform=video_augmentation_val, stride=args.stride, clip_len=args.clip_len)
    ...
    if args.rank == 0:
        ...
        wandb.init(project=args.wandb_project_name, entity="franklin_wang", name=(args.run_name +'_lincls_'+ str(pretrained_epoch) ))
        wandb.config = {
...
def train(train_loader, model, criterion, optimizer, summary_writer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.rank == 0:
            summary_writer.add_scalar("train_loss", loss.item(), epoch * iters_per_epoch + i)
            summary_writer.add_scalar("train_acc1", acc1[0], epoch * iters_per_epoch + i)
            summary_writer.add_scalar("train_acc5", acc5[0], epoch * iters_per_epoch + i)
            wandb.log({"train_loss": loss.item(), "train_acc1" : acc1[0], "train_acc5": acc5[0]}, epoch * iters_per_epoch + i)
...
def validate(val_loader, model, criterion, summary_writer, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
